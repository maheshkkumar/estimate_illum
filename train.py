...
def evaluate_model(net, loader, device, opts):
    ...
    return l1_metric.avg, np.sqrt(
        mse_metric.avg), psnr_metric.avg, ssim_metric.avg
...
def main(opts):
    ...
    for epoch in range(opts.epochs):
        ...
        for didx, data in enumerate(trainloader):
            ...
            total_loss = (opts.theta * loss) + \
                (1.0 * loss_edges) + (1.0 * ssim_loss)
            total_loss.backward()
            optimizer.step()
            ...
            if (didx + 1) % opts.print_frequency == 0:
                ...
                print("Epoch: [{}]/[{}], Iteration: [{}]/[{}], Total Loss: {:.4f}, Loss: {:.4f}, SSIM: {:.4f}, Gradient Loss: {:.4f}, Time: {:.2f}s".format(
                    epoch + 1, opts.epochs, didx + 1, len(trainloader), total_loss.item(), loss.item(), ssim_loss.item(), loss_edges.item(), time_taken))
                ...
                if (didx + 1) % opts.save_images == 0:
                    ...
                    # if shift range is defined, then shift all the data to be
                    # in [0, 1] and clamp the values
                    if opts.shift_range:
                        img = torch.clamp(img + 0.5, 0, 1)
                        probe = torch.clamp(probe + 0.5, 0, 1)
                        pred = torch.clamp(pred + 0.5, 0, 1)
                    ...
                    writer.add_image(
                        'train/Input Probes', input_probes, counter)
                    writer.add_image(
                        'train/Output Probes', output_probes, counter)
        ...
        if (epoch + 1) % opts.validate_every == 0:
            ...
            eval_start = time()
            l1_error, rmse_error, psnr_value, ssim_value = evaluate_model(
            ...
            print("Val Epoch: [{}]/[{}], MAE: {:.4f}, RMSE: {:.4f}, PSNR: {:.4f}, SSIM: {:.4f} Time: {:.2f}s".format(
                epoch + 1, opts.epochs, l1_error, rmse_error, psnr_value, ssim_value, time() - eval_start))
            ...
            if rmse_error < best_rmse or l1_error < best_l1 or psnr_value > best_psnr or ssim_value > best_ssim:
                ...
                model_snapshot_name = os.path.join(
                    model_path, "epoch_{}_mae_{:.2f}_rmse_{:.2f}_psnr_{:.2f}_ssim_{:.2f}.pth".format(
                        epoch + 1, l1_error, rmse_error, psnr_value, ssim_value))
