...
# epochs=90
# date=$(date '+%d_%m_%Y_%H_%M_%S');
# experiment="${date}_experiment_${loss}_${model}_adversarial_theta_${theta}_ep_${epochs}"
# out="/project/aksoy-lab/datasets/Multi_Illum_Invariance/CMPT726/experiments/${experiment}"
# srun python train.py -p ${path} -o ${out} --batch_size ${batch_size} --learning_rate ${learning_rate} --chromesz ${chromesz} --loss_type ${loss} --crop_images --mask_probes --shift_range --model ${model} --theta ${theta} --epochs ${epochs}
...
experiment="${date}_experiment_${loss}_${model}_adversarial_theta_${theta}_ep_${epochs}"
out="/project/aksoy-lab/datasets/Multi_Illum_Invariance/CMPT726/experiments/${experiment}"
srun python train_adversarial.py -p ${path} -o ${out} --batch_size ${batch_size} --learning_rate ${learning_rate} --chromesz ${chromesz} --loss_type ${loss} --crop_images --mask_probes --shift_range --model ${model} --theta ${theta} --epochs ${epochs}
