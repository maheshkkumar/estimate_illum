...
def compute_loss(criterion, gradient_criterion, ssim, prediction, ground_truth, opts):
...
def evaluate_model(net, loader, device, opts):
    ...
    with torch.no_grad():
        for didx, data in enumerate(loader):
            ...
            source_prob_pred = source_prob_pred.reshape(source_prob_pred.shape[0], 3, opts.chromesz, opts.chromesz)
            ...
            source_probe_l1_metric.update(F.l1_loss(source_prob_pred, source_probe).item())
            source_probe_mse_metric.update(F.mse_loss(source_prob_pred, source_probe).item())
            source_probe_psnr_metric.update(psnr(source_prob_pred, source_probe).item())
            source_probe_ssim_metric.update(ssim(source_prob_pred, source_probe).item())

            relit_l1_metric.update(F.l1_loss(source_relit, target_img).item())
            relit_mse_metric.update(F.mse_loss(source_relit, target_img).item())
            relit_psnr_metric.update(psnr(source_relit, target_img).item())
            relit_ssim_metric.update(ssim(source_relit, target_img).item())

    return (source_probe_l1_metric.avg, source_probe_mse_metric.avg, source_probe_psnr_metric.avg, source_probe_ssim_metric.avg), (relit_l1_metric.avg, relit_mse_metric.avg, relit_psnr_metric.avg, relit_ssim_metric.avg)
...
def main(opts):
    ...
    for epoch in range(opts.epochs):
        ...
        for didx, data in enumerate(trainloader):
            ...
            source_relit, source_prob_pred, non_light_features = net(source_img, target_probe)
            source_prob_pred = source_prob_pred.reshape(source_prob_pred.shape[0], 3, opts.chromesz, opts.chromesz)

            # source probe loss
            probe_total_loss, probe_loss, probe_grads, probe_ssim = compute_loss(criterion, gradient_criterion, ssim, source_prob_pred, source_probe, opts)
            
            # relit image loss
            img_total_loss, img_loss, img_grads, img_ssim = compute_loss(criterion, gradient_criterion, ssim, source_relit, target_img, opts)

            # if non_light_loss is True
            target_probe_loss, target_loss, target_grads, target_ssim = torch.Tensor([0]).to(device), torch.Tensor([0]).to(device), torch.Tensor([0]).to(device), torch.Tensor([0]).to(device)
            non_light_feat_loss = torch.Tensor([0]).to(device)
        
            if opts.non_light_loss:
                _, target_prob_pred, target_non_light_features = net(source_relit, None)
                target_prob_pred = target_prob_pred.reshape(target_prob_pred.shape[0], 3, opts.chromesz, opts.chromesz)
                target_probe_loss, target_loss, target_grads, target_ssim = compute_loss(criterion, gradient_criterion, ssim, target_prob_pred, target_probe, opts)
                non_light_feat_loss = criterion(target_non_light_features, non_light_features)

            total_loss = probe_total_loss + img_total_loss + target_probe_loss + non_light_feat_loss
            total_loss.backward()
            optimizer.step()
            ...
            if (didx + 1) % opts.print_frequency == 0:
                ...
                print("Epoch: [{}]/[{}], Iteration: [{}]/[{}], Total Loss: {:.4f}, Probe loss: {:.4f}, Image loss: {:.4f}, Target probe loss: {:.4f}, Non-light loss: {:.4f} Time: {:.2f}s".format(epoch + 1, opts.epochs, 
                                                                    didx + 1, len(trainloader), total_loss.item(), probe_total_loss.item(), img_total_loss.item(), target_probe_loss.item(), non_light_feat_loss.item(), time_taken))
                writer.add_scalar('train/Learning Rate', optimizer.param_groups[0]['lr'], counter)
                wandb.log({"train/lr": optimizer.param_groups[0]['lr']})
                writer.add_scalar('train/total_loss', total_loss.item(), counter)
                wandb.log({"train/total_loss": total_loss.item()})
                writer.add_scalar('train/probe_loss', probe_total_loss.item(), counter)
                wandb.log({"train/probe_loss": probe_total_loss.item()})
                writer.add_scalar('train/img_loss', img_total_loss.item(), counter)
            ...
            if (didx + 1) % opts.save_images == 0:
                ...
                if opts.shift_range:
                    ...
                    if opts.non_light_loss:
                        target_prob_pred = torch.clamp(target_prob_pred + 0.5, 0, 1)

                source_imgs = torchvision.utils.make_grid(source_img)
                source_probes = torchvision.utils.make_grid(source_probe)
                source_prob_preds = torchvision.utils.make_grid(source_prob_pred)
                ...
                wandb.log({"train/source images" : [wandb.Image(i) for i in source_img]})
                writer.add_image('train/Source Probes', source_probes, counter)
                wandb.log({"train/source probes" : [wandb.Image(i) for i in source_probe]})
                writer.add_image('train/Source Output Probes', source_prob_preds, counter)
                wandb.log({"train/source output probes" : [wandb.Image(i) for i in source_prob_pred]})

                writer.add_image('train/Target Images', target_imgs, counter)
                wandb.log({"train/target images" : [wandb.Image(i) for i in target_img]})
                writer.add_image('train/Target Probes', target_probes, counter)
                wandb.log({"train/target probes" : [wandb.Image(i) for i in target_probe]})
                writer.add_image('train/Source relit', source_relits, counter)
                wandb.log({"train/source relit" : [wandb.Image(i) for i in source_relit]})

                if opts.non_light_loss:
                    target_prob_preds = torchvision.utils.make_grid(target_prob_pred) 
                    writer.add_image('train/Target Output Probes', target_prob_preds, counter)
        ...
        if (epoch + 1) % opts.validate_every == 0:
            ...
            eval_start = time()
            (probe_l1_error, probe_rmse_error, probe_psnr_value, probe_ssim_value), (img_l1_error, img_rmse_error, img_psnr_value, img_ssim_value) = evaluate_model(net, valloader, device, opts)
            ...
            print("Val Epoch: [{}]/[{}], Probe MAE: {:.4f}, RMSE: {:.4f}, PSNR: {:.4f}, SSIM: {:.4f}, Time: {:.2f}s".format(epoch + 1, opts.epochs, probe_l1_error, probe_rmse_error, probe_psnr_value, probe_ssim_value, time()-eval_start))
            print("Val Epoch: [{}]/[{}], Image MAE: {:.4f}, RMSE: {:.4f}, PSNR: {:.4f}, SSIM: {:.4f}, Time: {:.2f}s".format(epoch + 1, opts.epochs, img_l1_error, img_rmse_error, img_psnr_value, img_ssim_value, time()-eval_start))
            ...
            if probe_rmse_error < best_probe_rmse or probe_l1_error < best_probe_l1 or probe_psnr_value > best_probe_psnr or probe_ssim_value > best_probe_ssim or img_rmse_error < best_img_rmse or img_l1_error < best_img_l1 or img_psnr_value > best_img_psnr or img_ssim_value > best_img_ssim:
                ...
                model_name = "epoch_{}_pmae_{:.2f}_prmse_{:.2f}_ppsnr_{:.2f}_pssim_{:.2f}_imae_{:.2f}_irmse_{:.2f}_ipsnr_{:.2f}_issim_{:.2f}.pth".format(epoch + 1, probe_l1_error, probe_rmse_error, probe_psnr_value, probe_ssim_value, img_l1_error, img_rmse_error, img_psnr_value, img_ssim_value)
